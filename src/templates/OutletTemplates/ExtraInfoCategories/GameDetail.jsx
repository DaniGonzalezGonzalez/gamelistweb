...
import { addDocument, getDocument, getDocumentsWithFilter, getGameByTitle } from "../../../api/supabase/cloud-supabase"
...
import { EditEstadoPanel, EditEstadoPanelAddGame, EditNotaPanel, EditRejugandoPanel } from "../../helpers/components/Menus&IndexHelpers/EditsNotaEstadoRejugando"
...
import { PanelAddEstadoFicha } from "../../helpers/components/Menus&IndexHelpers/EditsNotaEstadoRejugando/PanelAddEstadoFicha"
import { EditPlatformPanel } from "../../helpers/components/Menus&IndexHelpers/EditsNotaEstadoRejugando/EditPlatformPanel"
import { EditPlatformPanelFicha } from "../../helpers/components/Menus&IndexHelpers/EditsNotaEstadoRejugando/EditPlatformPanelFicha"
...
export function GameDetail() {
    ...
    const { handleOpenEditNotaPanel, handleCloseEditNotaPanel, editNotaPanelOpen, handleCloseEditEstadoPanel, editEstadoPanelOpen, handleOpenEditEstadoPanel, handleCloseRejugandoPanel, handleOpenRejugandoPanel, editRejugandoPanelOpen, handleEstadoChange, handleNotaJuegoChange, handlePosition, handleRejugandoChange, estado, notaJuego, rejugando, position, setEstado, setNotaJuego, setPosition, setRejugando, error, setError, panelAddEstadoFichaOpen, setPanelAddEstadoFichaOpen, editPlatformPanelOpen, setEditPlatformPanelOpen, handleOpenEditPlatformPanel, selectedPlatforms, handleCloseEditPlatformPanel, plataforma, setPlataforma } = useHandles(gameId)
    ...
    }, [gameId, platformSelected])
    
    useEffect(() => {        
    }, [platformSelected]); // Sólo depende de platformSelected
        ...
        <div className={`w-full min-h-screen pt-14 sm:pt-10 lg:pt-32 ${backgroundClass} flex flex-col items-center`}>
            ...
            <div className="relative z-10 flex justify-center w-full top-10">
                <div className="relative">
                    ...
                    {estado && <button onClick={handleOpenEditEstadoPanel} className="flex flex-col items-center justify-center gap-3 mt-6 text-[9px] lg:text-[11px] absolute  bottom-2 right-2 text-white">
                    ...
                    {!estado && <button type="button"  onClick={handleOpenPanel} disabled={isLoading || !juego?.titulo} className="flex flex-col items-center justify-center gap-3 mt-6 text-[9px] lg:text-[11px] absolute  bottom-2 right-2 text-white">
                        <div className={`${GET_STATE_BACKGROUND(estado)} px-1.5 py-1 lg:px-2 lg:py-1 lg:rounded-lg rounded-lg flex items-center`}>
                            <p className="lg:mr-1.5 mr-1 flex items-center gap-1"> <PlusIcon w={4} h={4} /> Añadir</p>
                            {estados.map(option => (
                                <div key={option}>{option === estado && estadoIconos[option]}</div>                                
                            )
                            )}
            ...
            <div className="w-full px-5 text-white bg-black shadow-lg py-14 lg:py-20">
                <div className="max-w-screen-lg mx-auto text-center">
                    ...
                    { collection==='Juegos' && 
                    <div className="flex items-center justify-center mt-2" onClick={() => handleOpenEditPlatformPanel(juego.plataforma)}>
                            ...
                            if (cleanedPlatform === 'null' || formattedPlatform === cleanedPlatform.replace(/\s+/g, '-').trim()) {
                                return (
                                    <img 
                                        ...
                                        alt={plataforma} 
                                        className="object-contain p-1 bg-gray-300 rounded-lg w-7 h-7 sm:w-8 sm:h-8" 
                                    />
                                );
                            }
                    ...
                    <div className="flex justify-center gap-10 mt-8 lg:mt-10">
                        <div className="flex items-center justify-center gap-4 sm:gap-4">
                            {estado && <div className="flex flex-col items-center justify-center gap-2">
            ...
            {editPlatformPanelOpen && <EditPlatformPanelFicha onClose={handleCloseEditPlatformPanel} textoBoton='Avanzar' onPlatformChange={handlePlatformChange} onAvanzar={handleEditEstado} onOmitir={true} platformActual={selectedPlatforms} tituloJuego={juego?.titulo} juegoId={gameId} isFromAddFicha={juego.position}/>  } 
            ...
            {panelAddEstadoFichaOpen && (
      <PanelAddEstadoFicha
        // onClose={() => setPanelAddEstadoFichaOpen(false)}
        onEstadoChange={handleEstadoSeleccionado}
        ...
        platformDefault={cleanedPlatform}
      />
    )}
