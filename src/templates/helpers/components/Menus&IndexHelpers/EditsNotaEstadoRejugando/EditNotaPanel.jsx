...
export function EditNotaPanel({ onClose, onNotaChange, onAvanzar, textoBoton, onOmitir, notaActual, onAdded }) {
  ...
  // useEffect(() => {
  //   onNotaChange(nota); // Llama a la función de callback para enviar la nota al padre
  // }, [nota, onNotaChange]);
      ...
      >
        ...
        <div className="grid grid-cols-4 gap-2 sm:grid-cols-8">
          {options.map((option) => (
            <button
              key={option.value}
              onClick={() => handleNotaClick(option.value)}
              className={`p-3 hover:bg-gray-500 rounded-lg text-white border-2 border-black ${option.color} ${onAdded && (Number(nota) === Number(option.value)) && 'border-blue-500 border-2'}`}
            >            
              {option.value}
            </button>
          ))}
