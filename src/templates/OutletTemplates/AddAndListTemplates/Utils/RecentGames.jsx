...
export function RecentGames({ recentGames, handleGameSelect, byPlatform, setEditEstadoPanelOpen, onAvanzar, user }) {
  ...
  return (
    <>
     <h2 className="w-full mt-3 text-lg font-bold text-white lg:mt-4 lg:w-5/6">Recién llegados</h2>
      <ul className="grid w-full grid-cols-2 gap-4 pt-4 mb-6 text-xs text-white border-t border-gray-700 lg:mt-2 lg:gap-3 lg:w-5/6 sm:mb-10 xl:grid-cols-7 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3">
        {recentGames.map((game) => {
          // Obtenemos los juegos coincidentes y las plataformas para cada juego
          const juegosCoincidentes = obtenerJuegosCoincidentes(game, dataBD);
          const plataformas = getPlataformas(juegosCoincidentes);
          const estaAñadido = juegosCoincidentes.length > 0 // Si hay coincidencias, significa que el juego está añadido
