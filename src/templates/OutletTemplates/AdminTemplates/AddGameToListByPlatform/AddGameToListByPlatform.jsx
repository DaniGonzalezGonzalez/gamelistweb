...
import { cleanTitle, getPlatformBackground, scrollToTop, totalNotaMetacriticPrensa, totalTiempoMainStory } from "../../../helpers/no-components/constants"
...
export function AddGameToListByPlatform() {
  ...
  // const [platformImageUrlTop, setPlatformImageUrlTop] = useState('');
  ...
  // useEffect(() => {
  //   const loadPlatformImageTop = async () => {
  //     const url = await getPlatformImageUrl(platform);
  //     setPlatformImageUrlTop(url);
  //   };

  //   loadPlatformImageTop();
  // }, [platform]);
...
}, [searchTerm, gamesBDByPlatform, platform, selectedTitle])
  ...
  return (
    ...
    <div className="flex flex-col items-start justify-start min-h-screen bg-slate-950">
     {/* <div className="flex flex-col items-start justify-start w-full min-h-screen p-10 pt-40 bg-slate-950" style={{ backgroundImage: `url("/Imagen-fondo-colecciones.jpg")`, backgroundSize: 'cover', backgroundPosition: 'center'}} > */}
          {/* <h1 className="flex justify-center w-full pb-5 text-2xl font-bold text-white uppercase lg:pb-10">Añadir juegos</h1> */}
        <div className="flex flex-col items-center justify-between w-full">
          <div className={`flex flex-col justify-center w-full text-center ${getPlatformBackground(platform)} pt-28`}>
            <div className="relative flex justify-center mb-5">
              <div className="w-80 sm:w-96">
                <button onClick={() => {tituloRef.current.focus()}} className="relative w-80 sm:w-96">
                  {/* <ImagesHeadAddToGameList gamesBDByPlatform={gamesBDByPlatform}/> */}

                  {/* <img className="object-cover w-full h-32 rounded-lg sm:h-60" src={gamesBDByPlatform[Math.floor(Math.random()*(gamesBDByPlatform.length))]?.url[0]?? gamesBDByPlatform[Math.floor(Math.random()*(gamesBDByPlatform.length))]?.imageUrl } alt="No hay imagen" /> */}
                  ...
                  <div className="absolute inset-0 flex items-end justify-end p-3 font-semibold text-center text-white">
                    ...
                    <div className="text-xs sm:text-sm">{platform}</div>
                  </div>
                </button>
              </div>
            </div>          
          ...
          <form className="flex flex-col items-center w-full" onSubmit={handleSubmit}>
            <div className="flex flex-col px-10 sm:mt-10">
              <div className="flex flex-col items-start justify-between w-full gap-6 sm:gap-3">
                      ...
                      <li key={game.titulo}  className="px-4 py-2 cursor-pointer">
                        <div className="h-full transition duration-300 w-28 sm:w-32 hover:scale-105">
                          <div className="relative flex items-center justify-center gap-3 shadow-md sm:flex hover:rounded hover:shadow-gray-700">
                              ...
                              {/* <img className="absolute object-contain w-8 h-8 p-1 bg-gray-200 rounded-lg shadow right-2 bottom-2 shadow-black" src={game?.url[1]??game.platformImageUrl} alt="No hay imagen" title={`Plataforma: ${game?.plataforma || 'Sin plataforma especificada'}`}></img> */}
                ...
                // : (
                  ...
                  //       <img className="object-cover w-full rounded-lg h-28 sm:h-60" src={gamesBDByPlatform[Math.floor(Math.random()*(gamesBDByPlatform.length))]?.url[0]?? gamesBDByPlatform[Math.floor(Math.random()*(gamesBDByPlatform.length))]?.imageUrl } alt="No hay imagen" />
                  //       <div className="absolute inset-0 bg-black rounded-lg opacity-60"></div>
                  ...
                  //       </p>
                  //       <p className="absolute inset-0 flex items-end justify-end p-3 font-semibold text-center text-white">
                  //         {platform}
                  //       </p>
                  //     </button>
                  //   </div>
                ...
                <div className='flex items-center justify-center w-full gap-2 mt-8 sm:mt-10 sm:gap-4'>
                  ...
                  <select className="w-full p-1 text-xs text-center text-white bg-gray-700 border-2 rounded appearance-none sm:p-2 sm:w-52 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500" name="estado" id="estado">
                        ...
                        {/* <option className="text-white bg-violet-600" value='Proximo'>Próximo</option> */}
                        <option className="text-center text-white bg-blue-800" value='En lista'>En lista</option>
                        {/* <option className="text-white bg-green-500" value='Recién terminado'>Recién terminado</option> */}
                        <option className="text-center text-white bg-green-900" value='Terminado'>Terminados</option>
                        <option className="text-center text-white bg-gray-600" value='Completando'>Completando</option>
                        <option className="text-center text-white bg-orange-600" value='Lista de deseos'>Lista de deseos</option>
                        <option className="text-center text-white bg-purple-800" value='Rejugar'>Rejugar</option>
                        <option className="text-center text-white bg-yellow-700" value='Pausado'>Pausado</option>
                        <option className="text-center text-white bg-red-800" value='Abandonado'>Abandonado</option>
                  </select>                 
                </div>
                ...
                {/* Envío de información en modo oculto */}
                ...
                <input className="w-full p-2 border rounded" type="text" hidden name="plataforma" id="plataforma" value={plataforma} onChange={handleEmptyChange}/>
                ...
                <input className="w-full p-2 border rounded" type="text" hidden name="descripcion" id="descripcion" value={descripcion} onChange={handleEmptyChange}/>
                <input className="w-full p-2 border rounded" type="number" hidden name="notaMetacriticPrensa" id="notaMetacriticPrensa" value={notaMetacriticPrensa} onChange={handleEmptyChange}/>
                <input className="w-full p-2 border rounded" type="number" hidden name="notaMetacriticUsuarios" id="notaMetacriticUsuarios" value={notaMetacriticUsuarios} onChange={handleEmptyChange}/>
                <input className="w-full p-2 border rounded" type="number" hidden name="tiempoMainStory" id="tiempoMainStory" value={tiempoMainStory} onChange={handleEmptyChange}/>
                <input className="w-full p-2 border rounded" type="number" hidden name="tiempoMainAndSides" id="tiempoMainAndSides" value={tiempoMainAndSides} onChange={handleEmptyChange}/>
                <input className="w-full p-2 border rounded" type="number" hidden name="tiempoCompletionist" id="tiempoCompletionist" value={tiempoCompletionist} onChange={handleEmptyChange}/>
                <input className="w-full p-2 border rounded" type="text" hidden name="linkMetacritic" id="linkMetacritic" value={linkMetacritic} onChange={handleEmptyChange}/>              
                <input className="w-full p-2 border rounded" type="text" hidden name="linkHowLongToBeat" id="linkHowLongToBeat" value={linkHowLongToBeat} onChange={handleEmptyChange}/>
                <input className="w-full p-2 border rounded" type="text" hidden name="datosExtraJuego" id="datosExtraJuego" value={datosExtraJuego} onChange={handleEmptyChange}/>
                <input className="w-full p-2 border rounded" type="text" hidden name="genero" id="genero" value={genero} onChange={handleEmptyChange}/>
                ...
                <div className='flex flex-col gap-2'>
                  ...
                  <textarea className="p-2 border rounded resize-none" name="infouser" id="infouser" placeholder={user.email} rows="3" cols="33" hidden onChange={handleEmptyChange} value={user.email}></textarea>
                </div>
              </div>

              <p className="px-2 py-1 mt-6 text-sm font-bold text-center text-white bg-green-700 rounded-lg sm:mt-14" hidden={isLoading || !isTitleValid || gameAdded }>¡{cleanTitle(selectedTitle)} seleccionado correctamente! Haz click en Añadir juego para incluirlo en la colección deseada</p>
              ...
              {/* <p className="flex flex-col items-center justify-between mt-5 text-xs text-white sm:mt-10">Añádelo a tu colección</p> */}
