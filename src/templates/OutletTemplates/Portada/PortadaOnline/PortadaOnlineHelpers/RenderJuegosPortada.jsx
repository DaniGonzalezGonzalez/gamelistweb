...
export const RenderJuegosPortada = (juegos, platformImages, handleTitleClick) => {
    return juegos.map((item, index) => (
        <div key={index}>
            <div className="flex items-center justify-center gap-1 px-2 duration-500 sm:flex hover:scale-102 hover:shadow-white opacity-95 hover:opacity-100">
                <div className="w-full h-full">
                    <button onClick={() => handleTitleClick(item.id)} className="relative flex items-center justify-center w-full gap-3 shadow-md sm:flex hover:rounded">
                        {/* <div className="relative w-full h-40 img-container"> */}
                            <img className="object-cover w-full h-40 transition duration-500 ease-in-out border-2 border-transparent rounded-lg sm:h-28 md:h-40 img-container hover:border-2 hover:rounded-lg hover:border-gray-300" src={item.imageUrl ?? item.url[0]} alt="No hay imagen" />
                        {/* </div> */}
