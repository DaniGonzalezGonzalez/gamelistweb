...
import { handlePlatformClick, useCheckDuplicatePlatforms, useSplitPlatform } from '../Helpers/UseEffects'
import { useEffectFetchDataPlatform } from '../Helpers/UseEffects/useEffectFetchDataPlatform'
import { limpiarTituloJuego } from '../Helpers/utils'

export function EditPlatformPanel({ onClose, onPlatformChange, onAvanzar, textoBoton, onOmitir, platformActual, onAdded, tituloJuego, collection, isFromAddFicha, juegoId }) {
  ...
  useCheckDuplicatePlatforms({ tituloJuego, platform, user, loading, plataformasExistentes, errorMessage, setPlataformasExistentes,  setErrorMessage, limpiarTituloJuego, platformsList })
    ...
    <div ref={panelRef} className="flex flex-col items-center w-5/6 px-3 pt-5 pb-10 text-gray-100 border-2 border-gray-100 shadow-lg sm:w-4/6 lg:px-3 lg:pt-5 lg:pb-20 lg:w-1/2 rounded-2xl bg-slate-950" onClick={(e) => e.stopPropagation()}>
      ...
      `}>
        {platformsList.map((option) => {
          ...
          return (
            <button key={option} onClick={() => handlePlatformClick(option, platformsList, plataformasExistentes, setErrorMessage, setPlatform, tituloJuego, limpiarTituloJuego, collection, juegoId, updateDocument, onPlatformChange, onAvanzar, onClose, isFromAddFicha, true)} className={`flex flex-col items-center p-3 hover:bg-gray-500 transition duration-300 rounded-2xl text-white border-2 w-28 sm:w-16 lg:w-40 bg-gray-300`}>
