...
export function EditEstadoPanelAddGame({ onAvanzar, onClose, onEstadoChange, onAddGame, textoBoton, id, titulo, tabla, onAdded, estadoActual, onPosition, platform, formRef, handleSubmit, onNewTitulo  }) {
...
const newTitulo = platform ? `${titulo} - ${platform}` : titulo;
...
const handleCerrarPanel = () => {
  if (estado !== estadoActual) {
    onEstadoChange(estado); // Cambia el estado
  }

  onPosition(newPosition); // Cambia la posición
  onNewTitulo(newTitulo); // Cambia la posición

  if (formRef.current) {
    // Espera un ciclo de evento para asegurarte de que el formulario está actualizado
    setTimeout(() => {
      // Llama manualmente a handleSubmit
      handleSubmit({
        preventDefault: () => {}, // Evita que intente prevenir el comportamiento por defecto
        target: formRef.current,  // Pasa el formulario como target
      });
    }, 0);
  }

  onAvanzar(); // Cierra el panel
};
